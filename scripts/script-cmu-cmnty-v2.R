...
df <- smooth_column(df, "pct_cli", smooth_param, link_in ="log", monotone = F)
df <- smooth_column(df, "pct_cli_anosmia_ageusia", smooth_param, link_in ="log", monotone = F)
df$nsum_roll <- zoo::rollmean(df$nsum, 3, fill=NA)
df <- smooth_column(df, "nsum_roll", smooth_param, link_in ="log", monotone = F)
df$pct_cases_active_cdc <- df$cases_active_cdc/df$population*100
df <- smooth_column(df, "pct_cases_active_cdc", smooth_param, link_in ="log", monotone = F)
...
p <- ggplot(data = df, aes(x = date)) +
        ...
        geom_point(aes(y = pct_cli_anosmia_ageusia, color = "Cli_anosmia_ageusia"), alpha = 0.4) +
        geom_line(aes(y = pct_cli_anosmia_ageusia_smooth, color = "Cli_anosmia_ageusia smooth"), size = 0.8) +
        ...
        scale_colour_manual(name= "", values = my_colors,
                            guide = guide_legend(override.aes = list(
                                    linetype = c("blank", "solid", "blank", "solid", "blank", "solid"),
                                    shape = c(1, NA, 1, NA, 1, NA)))) + 
        ylim(-0.1, ymax) +
