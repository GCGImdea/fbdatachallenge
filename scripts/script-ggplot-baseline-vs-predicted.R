...
for (lag_in in 1:7) {
  df_col_rm <- read.csv(file = paste0(path_symptom_lags,
                                      country_iso,
                                      "-cases-7-60-penTRUE-alpha0.5-rmccTRUE-rmth0.9-2020-09-10-2020-11-10-1-TRUE-TRUE0-FALSE-estimates-lag-daily.csv")) %>%
    mutate(date = as.Date(date)) %>% 
    filter(lag == lag_in & predType == "nearFuture") 
  ...
  df_no_col_rm <- read.csv(file = paste0(path_symptom_lags,
                                         country_iso,
                                         "-cases-7-60-penTRUE-alpha0.5-rmccFALSE-rmth0.9-2020-09-10-2020-11-10-1-TRUE-TRUE0-FALSE-estimates-lag-daily.csv")) %>% 
    mutate(date = as.Date(date)) %>% 
    filter(lag == lag_in & predType == "nearFuture") 
